...
import com.wp.zxing.view.ViewfinderResultPointCallback;
...
public final class CaptureActivityHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        switch (message.what) {
            case Config.restart_preview: // 准备进行下一次扫描
            ...
            case Config.decode_failed:
                ...
                if (curTime - system_time > TIMEOUT) {
                    activity.timeOutData();
                    quitSynchronously();
                } else {
                    state = State.PREVIEW;
                    cameraManager.requestPreviewFrame(decodeThread.getHandler(), Config.decode);
                }
