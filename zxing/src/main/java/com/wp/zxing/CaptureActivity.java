...
import com.wp.zxing.camera.CameraManager;
import com.wp.zxing.config.Config;
import com.wp.zxing.decode.CaptureActivityHandler;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, View.OnClickListener {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case PARSE_BARCODE_SUC: // 解析图片成功
                    parseData(activityReference.get(), String.valueOf(msg.obj));
                    break;
    ...
    //解析数据并返回
    private static void parseData(Activity activity, String data) {
        Log.i(TAG, String.format("QRCode识别的数据:%s", data));
    ...
    //解析数据并返回
    public void timeOutData() {
        Log.i(TAG, "QRCode扫描超时。。。。。");
        Intent intent = new Intent();
        intent.putExtra("QRCode", Config.decode_time_out);
    ...
    private void jumpQrcode() {
        Log.i(TAG, "QRCode跳过扫码。。。。。");
        Intent intent = new Intent();
        intent.putExtra("QRCode", Config.jump_saoma);
        this.setResult(RESULT_OK, intent);
        this.finish();
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (resultCode == ImagePicker.RESULT_CODE_ITEMS) {
            ...
            switch (requestCode) {
                case REQUEST_CODE:
                    if (intent != null) {
                        ArrayList<ImageItem> imageItems = (ArrayList<ImageItem>) intent.getSerializableExtra(ImagePicker.EXTRA_RESULT_ITEMS);
                        if (imageItems != null && imageItems.size() > 0) {
                            photoPath = (imageItems.get(0).path);
                        }
                    }
    ...
    public void handleDecode(Result rawResult, Bitmap barcode, float scaleFactor) {
        ...
        parseData(this, ResultParser.parseResult(rawResult).toString());
    }
    ...
    private void decodeOrStoreSavedBitmap(Bitmap bitmap, Result result) {
        ...
        if (handler == null) {
            ...
            if (savedResultToShow != null) {
                Message message = Message.obtain(handler, Config.decode_succeeded, savedResultToShow);
    ...
    private void displayFrameworkBugMessageAndExit() {
        ...
        builder.setTitle(getString(R.string.app_saomiao));
    ...
    public void onClick(View v) {
        ...
        } else if (id == R.id.capture_button_cancel) {//取消按钮
            finish();
        } else if (id == R.id.capture_button_createcode) {
            jumpQrcode();
        }
    }
