...
public class ShopMallDetailActivity extends AndroidActivity implements View.OnClickListener {
    ...
    private ShopMallDetailEntity mShopMallDetailEntity;
    ...
    private void initViews() {
        ...
        findViewById(R.id.buy_shop).setOnClickListener(this);
...
//                getProductInfo();
    ...
    private void getProductList() {
        mShopAPI.getShopMallProductList(0, page, mShopId, mShouNewApplication.new ShouNewHttpCallBackLisener() {
            @Override
    ...
    private void getProductInfo() {
        mShopAPI.getShopMallProductInfo(mShopId, mShouNewApplication.new ShouNewHttpCallBackLisener() {
            @Override
            protected void resultData(Object data, JSONObject json, Response response, Exception exception) {
                if (exception == null) {
                    if (json.has("data")) {
                        try {
                            JSONObject jsonData = json.getJSONObject("data");
                            if (jsonData.has("mallproduct")) {
                                String product = jsonData.getString("mallproduct");
                                mShopMallDetailEntity = JsonUtils.fromJson(product, ShopMallDetailEntity.class);
                                if (mShopMallDetailEntity != null) {
                                    mBanner.setImages(mShopMallDetailEntity.getMpImg());
                                    if (mShopMallDetailEntity.getMpImg() != null) {
                                        int length = mShopMallDetailEntity.getMpImg().size();
                                    ...
                                    mBanner.start();
                                    mShopIntroTv.setText(mShopMallDetailEntity.getMpTitle());
                                    int mpColect = mShopMallDetailEntity.getMpCollect();
                                    ...
                                    mShopDetailPricesTv.setText("¥" + StringUtil.formatMoney(mShopMallDetailEntity.getMpPrice()));

                                    Spannable spanStrikethrough = new SpannableString(String.format("原价%s", StringUtil.formatMoney(mShopMallDetailEntity.getMpOldprice())));
                                    ...
                                    mKuaidiMoney.setText(String.format("快递:%s元", StringUtil.formatMoney(mShopMallDetailEntity.getMpKdprice())));
                                    mShopDetailAddress.setText(mShopMallDetailEntity.getMpAddress());
                                    List<String> imgs = mShopMallDetailEntity.getMpImgs();

                                    if (imgs != null && imgs.size() > 0) {
                                        mShopDetailImgParent.removeAllViews();
                                        for (String img : imgs) {
                                            final ImageView imageView = new ImageView(ShopMallDetailActivity.this);
...
//                    }
        ...
        switch (view.getId()) {
            ...
            case R.id.buy_shop:
                if (!Preference.getBoolean(ShopMallDetailActivity.this, Preference.IS_LOGIN, false)) {
                    mShouNewApplication.jumpLoginActivity(ShopMallDetailActivity.this);
                    return;
                }
                if (mShopMallDetailEntity != null) {
                    new ShopSelectDialog(ShopMallDetailActivity.this, mShopMallDetailEntity.getMpSimg(), mShopMallDetailEntity.getMpAllprice(), mShopMallDetailEntity.getMpColor()).setDialogLisener(new ShopSelectDialog.DialogLisener() {
                        @Override
                        public void sure(String color, int number, double prices) {
                            Bundle bundle = new Bundle();
                            bundle.putParcelable("mSuperMarkeDetailEntity", mShopMallDetailEntity);
                            bundle.putString("color", color);
                            bundle.putInt("number", number);
                            bundle.putDouble("prices", prices);
                            mShouNewApplication.redirectAndPrameter(ShopMallSureOderMenuActivity.class, bundle);
                        }
                ...
                if (mShopMallDetailEntity != null) {
                    new ShopSelectDialog(ShopMallDetailActivity.this, mShopMallDetailEntity.getMpSimg(), mShopMallDetailEntity.getMpAllprice(), mShopMallDetailEntity.getMpColor()).setDialogLisener(new ShopSelectDialog.DialogLisener() {
                        ...
                        public void sure(String color, int number, double prices) {
                            ...
                            shopCarEntity.setShMpid(Integer.parseInt(mShopMallDetailEntity.getMpId()));
                            shopCarEntity.setShNum(number);
                            shopCarEntity.setShPid(0);
                            ...
                            shopCarEntity.setShSimg(mShopMallDetailEntity.getMpSimg());
                            shopCarEntity.setShTitle(mShopMallDetailEntity.getMpTitle());
                            shopCarEntity.setShKdprice(mShopMallDetailEntity.getMpKdprice());

                            if (!Preference.getBoolean(mShouNewApplication, Preference.IS_LOGIN, false)) {
                                DatabaseUtils.insert(ShopMallDetailActivity.this, shopCarEntity);
    ...
    private void collection() {
        ...
        //  cannelCollection  取消
        //collect  收藏
        String collect = mShopMallDetailEntity.getMpCollect() == 0 ? "collect" : "cannelCollection";
        String shopId = mShopMallDetailEntity.getMpId();
        mShopAPI.collection(0, shopId, collect, mShouNewApplication.new ShouNewHttpCallBackLisener() {
            ...
            protected void resultData(Object data, JSONObject json, Response response, Exception exception) {
                if (exception == null) {
                    ...
                    mShopMallDetailEntity.setMpCollect(mpColect != 0 ? 0 : 1);
                    isFavor.setText(mpColect != 0 ? "未收藏" : "已收藏");
                    isFavor.setCompoundDrawablesWithIntrinsicBounds(0, mpColect != 0 ? R.drawable.collection_gary : R.drawable.collection_orange, 0, 0);
                } else {
