...
import static com.shownew.home.R.id.rg_sn;
import static com.shownew.home.R.id.rg_wx;
import static com.shownew.home.R.id.rg_zf;


/**
 * 确认订单
 */
public class ShopMallSureOderMenuActivity extends BaseActivity implements View.OnClickListener {
    private ShopMallDetailEntity mSuperMarkeDetailEntity;
    ...
    private void setData() {
        if (mSuperMarkeDetailEntity != null) {
            mKuaidi_fee.setText(String.format("快递：%s元", StringUtil.formatMoney(mSuperMarkeDetailEntity.getMpKdprice())));
            mShop_prices.setText(String.format("¥%s", StringUtil.formatMoney(prices)));

            mShop_title.setText(mSuperMarkeDetailEntity.getMpTitle());
            Glide.with(ShopMallSureOderMenuActivity.this).load(mSuperMarkeDetailEntity.getMpSimg()).asBitmap().diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.square_seize).error(R.drawable.square_seize).into(new SimpleTarget<Bitmap>() {
                @Override
    ...
    private void initRechargeWays() {
        ...
        rbSn = (RadioButton) findViewById(rg_sn);
    ...
    private void submitOrderz() {
        ...
        if (mSuperMarkeDetailEntity != null) {
            ...
            if (flag == 2) {
                if (tradePass == 0) {
                    new CommonDialog(ShopMallSureOderMenuActivity.this, "温馨提示!", "您还没有设置交易密码,是否设置?", "是", "否").setCommonListener(new CommonDialog.CommonListener() {
                        @Override
            ...
            mUserAPI.submitShopMallOrderz(locationId, StringUtil.formatMoney(money - mSuperMarkeDetailEntity.getMpKdprice()), note, mSuperMarkeDetailEntity.getMpId(), mColor, mNumber, flag, mShouNewApplication.new ShouNewHttpCallBackLisener() {
                @Override
                    ...
                    if (null != json) {
                        if (json.has("data")) {
                            try {
                                ...
                                WXPay.getInstans(ShopMallSureOderMenuActivity.this).WxPay(tenpayInfo);
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
...
//                                            mShouNewApplication.redirectAndPrameter(PayStateActivity.class, bundle);
    ...
    private void exitShow() {
        new CommonDialog(ShopMallSureOderMenuActivity.this, "温馨提示", "优惠不常在,下手要赶快!", "狠心离开", "我再想想").setCommonListener(new CommonDialog.CommonListener() {
            @Override
            public void sure(int flag) {
                if (1 == flag) {
                    finish();
                    ...
                    if (null != json) {
                        if (json.has("data")) {
                            try {
                                ...
                                new AlipayPayUtils(ShopMallSureOderMenuActivity.this, mHandler, tenpayInfo);
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
    ...
    private void shouniuCharge(final String orderzNo) {
        ...
        popEnterPassword.showAtLocation(ShopMallSureOderMenuActivity.this.findViewById(R.id.parent), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0); // 设置layout在PopupWindow中显示的位置;
...
//                                    }
                                    ...
                                    new CommonDialog(ShopMallSureOderMenuActivity.this, "余额不足,请充值").setCommonListener(new CommonDialog.CommonListener() {
                                        @Override
                                        public void sure(int flag) {
                                            if (1 == flag) {
