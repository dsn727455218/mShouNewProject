...
public class ShouNewApplication extends MainApplication implements Thread.UncaughtExceptionHandler {
  ...
  //在程序终止的时候执行
    ...
    public void loadImg(String imgUrl, final ImageView imageView) {
        Glide.with(instance).load(imgUrl).asBitmap().diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.square_seize).error(R.drawable.square_seize).into(new SimpleTarget<Bitmap>() {
            @Override
