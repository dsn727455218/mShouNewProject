...
public class CacheCall<T> implements Call<T> {
    ...
    public void execute(AbsCallback<T> callback) {
        ...
        if (baseRequest.getCacheKey() == null)
            baseRequest.setCacheKey(HttpUtils.createUrlFromParams(baseRequest.getParams().urlParamsMap, baseRequest.getBaseUrl()));
    ...
    private void handleCache(Headers headers, T data) {
        if (baseRequest.getCacheMode() == CacheMode.NO_CACHE)
            return;    //不需要缓存,直接返回
        ...
        OkGo.getInstance().getDelivery().post(new Runnable() {
            ...
            public void run() {
                if (isFromCache) {
                    ...
                    mCallback.resultDataUI(t, call, response);      //请求成功回调 （UI线程）
                    mCallback.onAfter(t, null);                  //请求结束回调 （UI线程）
                }
            }
        });
